...
import SpiralUp from "../../assets/GraphBG.png"
...
const WhatsSEO = () => {
  ...
  useEffect(() => {
    const lenis = new Lenis({
      duration: 1.2,       // Increased duration for smoother scroll inertia
      easing: (t) => Math.pow(t, 3), // Custom cubic easing
      smooth: true,
      direction: 'vertical',
      ...
      touchMultiplier: 2,
      infinite: false,
      // Higher damping helps smoothness (default ~0.1-0.2)
      damping: 0.8
    });
  ...
  // Use framer-motion scroll tracking relative to this section
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ["start end", "end start"]
  });

  // Horizontal slide transforms tied to scroll progress
  ...
  // Floating animation variants for badges (smoother & longer duration)
          ...
          >
            <div>
              <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-center lg:text-start  font-montserrat text-gray-700 mb-3 sm:mb-4 lg:mb-5">
                Why Your Business Need <span className="bg-gradient-to-r from-orange-400 via-pink-400 to-purple-500 bg-clip-text text-transparent font-['Playfair_Display',serif] italic">SEO Services</span> 
              </h2>
